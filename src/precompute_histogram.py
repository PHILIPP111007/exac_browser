...
def main(args):
    ...
    if args.output is None:
        args.output = args.vcf.replace(".vcf", ".hist.vcf")
    ...
    for line in f:
        ...
        if header is None:
            (
                print >> sys.stderr,
                "VCF file does not have a header line (CHROM POS etc.). Exiting.",
            )
            sys.exit(1)
...
def get_histogram_for_variant(
    vcf_line, metric="DP", num_bins=40, midpoints=True, variants_only=False
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--vcf", "--input", "-i", help="Input VCF file; may be gzipped", required=True
    )
