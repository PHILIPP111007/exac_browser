...
def add_transcript_coordinate_to_variants(db, variant_list, transcript_id):
    ...
    exons = sorted(
        lookups.get_exons_in_transcript(db, transcript_id),
        key=itemgetter("start", "stop"),
    )
    ...
    for variant in variant_list:
        ...
        for i, exon in enumerate(exons):
            if exon["start"] <= variant["pos"] <= exon["stop"]:
                variant["transcript_coordinates"][transcript_id] = (
                    exon_offsets[i] + variant["pos"] - exon["start"]
                )
...
def add_consequence_to_variant(variant):
    ...
    if (
        csq_order_dict[variant["major_consequence"]]
...
def get_protein_hgvs(annotation):
    ...
    if "%3D" in annotation["HGVSp"]:  # "%3D" is "="
        try:
            amino_acids = "".join(
                [protein_letters_1to3[x] for x in annotation["Amino_acids"]]
            )
...
def remove_extraneous_vep_annotations(annotation_list):
    return [
        ...
        if worst_csq_index(ann["Consequence"].split("&"))
        <= csq_order_dict["intron_variant"]
    ]
...
def order_vep_by_csq(annotation_list):
    ...
    return sorted(
        annotation_list, key=(lambda ann: csq_order_dict[ann["major_consequence"]])
    )
