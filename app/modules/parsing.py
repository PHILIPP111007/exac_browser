...
def get_variants_from_sites_vcf(sites_vcf):
    ...
    for line in sites_vcf:
        try:
            ...
            consequence_array = (
                info_field["CSQ"].split(",") if "CSQ" in info_field else []
            )
            annotations = [
            ...
            for i, alt_allele in enumerate(alt_alleles):
                vep_annotations = [
                    ann for ann in coding_annotations if int(ann["ALLELE_NUM"]) == i + 1
                ]
                ...
                variant["xstop"] = (
                    variant["xpos"] + len(variant["alt"]) - len(variant["ref"])
                )
                ...
                variant["quality_metrics"] = dict(
                    [(x, info_field[x]) for x in METRICS if x in info_field]
                )

                variant["genes"] = list(
                    {annotation["Gene"] for annotation in vep_annotations}
                )
                variant["transcripts"] = list(
                    {annotation["Feature"] for annotation in vep_annotations}
                )
            ...
            logger.info("Error parsing vcf line: " + line)
            traceback.print_exc()
            break
...
def get_snp_from_dbsnp_file(dbsnp_file):
    for line in dbsnp_file:
        ...
        snp = {"xpos": get_xpos(chrom, start), "rsid": rsid}
        yield snp
